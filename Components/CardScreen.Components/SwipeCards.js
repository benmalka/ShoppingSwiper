...
export default class SwipeCards extends Component {
  ...
  renderNoMoreCards() {
    ...
    return <View><Text>No Cards</Text></View>;
  }
  ...
  renderStack() {
    ...
    let depth = this.props.stackDepth > this.state.cards.length ? this.state.cards.length : this.props.stackDepth
    let cards = this.state.cards.slice(currentIndex[this.guid], currentIndex[this.guid] + depth).reverse();
    return cards.map((card, i) => {
      ...
      let scale = (i*0.05) + (1-(0.05*cards.length)) + 0.05;
      let lastScale = (i*0.05) + (1-(0.05*cards.length));
      ...
      let style = {
        ...
        //bottom: this.state.enter.interpolate({ inputRange: [0, 1], outputRange: [lastOffsetX, offsetX] }),
      ...
      if (i + 1 === cards.length) {
        ...
        return <Animated.View key={card[this.props.cardKey].toString()} style={[styles.card, animatedCardStyles]} {... this._panResponder.panHandlers}>
  ...
  renderCard() {
    try{
      ...
      let animatedCardStyles = { transform: [{ translateX }, { translateY }, { rotate }, { scale }], opacity ,collapsable: false};
